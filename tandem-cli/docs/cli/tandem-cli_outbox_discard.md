## tandem-cli outbox discard

Discard a FAILED message (irreversible; unblocks the aggregate)

```
tandem-cli outbox discard <id> [flags]
```
...
```
  -h, --help            help for discard
      --reason string   reason for the discard, recorded for audit (required)
```
