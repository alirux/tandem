...
/**
 * Runs against a real OpenTelemetry SDK behind the Micrometer Tracing bridge, exporting into memory, so
 * what is asserted is a genuinely exported span with a genuinely resolved parent — the one thing a
 * hand-written stand-in could not prove (HLD-tracing.md §6).
 */
class MicrometerTandemSpanRecorderTest {
    ...
    private final Propagator propagator = new OtelPropagator(
            ...
            tracerProvider.get("tandem-relay"));

    private final MicrometerTandemSpanRecorder recorder = new MicrometerTandemSpanRecorder(propagator);
    ...
    void GIVEN_a_row_carrying_the_producing_trace_WHEN_it_is_published_THEN_the_publish_joins_that_same_trace() {
        ...
        assertThat(exporter.getFinishedSpanItems()).singleElement().satisfies(span -> {
            ...
            assertThat(span.getName()).isEqualTo(MicrometerTandemSpanRecorder.PUBLISH_SPAN_NAME);
        });
    }
