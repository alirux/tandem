 ...
 * check is what keeps that true of the file as the contract grows.
 */
class ReferenceConfigurationTest {
    ...
    private final String reference = resource("/tandem-producer-reference.yml");
    ...
    private List<String> referencedKeys() {
        return matches(DOCUMENTED_KEY, reference, 0).stream().distinct().toList();
    }
