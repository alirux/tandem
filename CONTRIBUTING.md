...
| `tandem-admin` | Optional REST Admin API over the outbox and the relay (API-first, off by default). Reads, replay/discard, and relay control (status, pause/resume, per-bucket/per-worker observability, force-release) are all implemented; only the attempt-archive endpoints remain, deferred with the archive itself — see [IMPLEMENTATION-PLAN-admin-api.md](docs/IMPLEMENTATION-PLAN-admin-api.md). |
