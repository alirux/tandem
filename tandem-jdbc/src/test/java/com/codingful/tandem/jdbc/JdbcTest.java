...
/**
 * Pins the one behaviour every {@code Jdbc*} adapter in this package depends on {@link Jdbc#run} for:
 * a {@link SQLException} — from acquiring the connection, or from the operation itself, both reach the
 * same {@code catch} — never escapes an adapter method raw. Needs no Docker: {@code getConnection()}
 * against an unroutable address fails fast, which is enough to exercise the wrapping without a real
 * database (the pattern every {@code Jdbc*IT} used to repeat once per method — item 30).
 */
class JdbcTest {
    ...
    void GIVEN_the_database_is_unreachable_WHEN_a_void_operation_runs_THEN_the_sqlexception_is_wrapped_as_a_tandem_exception() {
        assertThatThrownBy(() -> Jdbc.run(unreachable, "the void operation", (Jdbc.SqlAction) conn -> { }))
                .isInstanceOf(TandemException.class)
    ...
    void GIVEN_a_custom_exception_mapper_WHEN_a_void_operation_fails_THEN_the_mapper_translates_it_instead_of_the_default_wrap() {
        ...
        assertThatThrownBy(() -> Jdbc.run(unreachable, (Jdbc.SqlAction) conn -> { }, e -> marker))
                .isSameAs(marker);
    }
